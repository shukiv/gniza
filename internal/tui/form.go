...
func (f *form) view(width int) string {
	...
	if f.err != "" {
		b.WriteString(styleBad.Render(pad(f.err, width-2)) + "\n\n")
	}
	for i := range f.fields {
		if !f.shown(i) {
			continue
		...
		case fieldChoice:
			...
			for j, c := range fld.choices {
				if j == fld.chosen {
					parts[j] = styleActive.Render(" " + c.label + " ")
				} else {
					parts[j] = " " + c.label + " "
				}
			}
		...
		if fld.kind == fieldToggle {
			b.WriteString(marker + control + "\n")
		} else {
			b.WriteString(marker + pad(fld.label, 18) + control + "\n")
		}
		if fld.help != "" && i == f.focus {
			b.WriteString("    " + styleDim.Render(fld.help) + "\n")
		}
	}
