...
// operator's choice rather than a panel's list (ADR 0025). The tables
// are drawn in the schedule form, where they say what one schedule
// covers and a fresh row ticked there is added on save; the terminal
// chooses with forms of its own that post the same fields. The Sources
// page lists what was chosen. The handlers serve the socket, the
// browser door and the terminal.
//
...
// chooseView is what the schedule form shows about choosing, and what
// the Sources page carries as data for the terminal (Candidates, when
// asked for with add=1).
type chooseView struct {
	Candidates panel.Candidates
	// FormError says why the candidates could not be read.
	FormError string
...
// Box is the box for one row: a chosen row posts the source's name, so
// the schedule covers it, and a fresh one posts its kind, to be added
// on save. When nothing has been chosen yet the databases and the
// containers come up ticked, since backing them up is what the operator
// came for, and the folders not, since the roots hold things like
// /opt/containerd too.
...
func (s *Server) handleAddSource(w http.ResponseWriter, r *http.Request) {
	...
	case len(added) == 0 && len(refused) == 0:
		s.refuseSource(w, r, "Tick something, or name a folder: nothing was chosen.")
	case len(added) == 0:
		s.refuseSource(w, r, fmt.Sprintf("Nothing was added. %s.", strings.Join(refused, "; ")))
	case len(refused) > 0:
...
// refuseSource says why nothing was added. The terminal keeps its form
// open with the reason when the answer is a page rather than a redirect,
// so a data request is answered with one; a script's post is sent to
// the Sources page with the reason.
func (s *Server) refuseSource(w http.ResponseWriter, r *http.Request, cause string) {
	if wantsData(r) {
		s.render(w, r, "accounts.html", "Sources", "accounts", accountsView{FormError: cause})
		return
	}
	s.redirect(w, r, "/accounts", "error", cause)
}
